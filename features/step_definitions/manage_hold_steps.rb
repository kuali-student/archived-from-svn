...
When(/^I search for a hold with a valid hold code$/) do
  @manage_hold = create ManageHoldData, :hold_name=> "Academically Ineligible", :hold_code=> "Acad02"
end
...
And(/^I edit that hold by changing the hold authorization$/) do
  on ManageHold do |page|
    ...
    page.hold_popup_search
    page.hold_popup_table_select(1)
    page.hold_code_input.set "Acad02"
    page.hold_add_org
    page.hold_auth_find_btn
    page.hold_popup_search
    page.hold_popup_table_select(2)
    page.hold_auth_apply
    page.hold_save
  end
end

Then(/^the hold is displayed in the catalog with the updated authorization$/) do
  on(ManageHold).get_hold_auth("Graduate Studies").nil?.should be_false
end
class ManageCourseOfferings < BasePage
  ...
  action(:list_all_courses){ |b| b.list_all_course_link.click; b.loading.wait_while_present }
  ...
  action(:show) { |b| b.frm.button(text: "Show").click; sleep 2; b.loading.wait_while_present }
  ...
  action(:edit_course_offering) { |b| b.edit_course_offering_link.click; b.loading.wait_while_present }
  ...
  value(:co_comments_link_count){ |b| b.co_comments_link.text[/\d+(?=\))/].to_i }
  ...
  def ao_comments_link_count(code,cluster_private_name = :default_cluster)
    ao_comments_link(code,cluster_private_name).text[/\d+(?=\))/].to_i
  end
  ...
  def codes_list(cluster_private_name = :default_cluster)
    ...
    begin
      ...
      codes.delete_if { |code| code.downcase == "code" }
